...
@section('content')
            ...
            <div class="flex flex-wrap justify-center gap-8">
                ...
                @foreach ($categories as $item)
                    <div class="category-item">
                        <a href="{{ $item->id }}" class="text-lg text-yellow-500 hover:text-gray-800 transition">
                            {{ $item->cat_name }}
                        </a>
                    </div>
                @endforeach
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 p-6">
        {{-- First set of images (Large images) --}}
        @php
            $imageCount = 0;
        @endphp
        @foreach ($Filtercategories as $item)
            @php
                $imagePath = public_path('images/' . $item->cat_image);
            ...
            @if ($imageWidth > 500 && $imageHeight > 500)
                <div
                    ...
                    <div
                        ...
                        <p class="text-white text-lg sm:text-xl font-semibold">{{ $item->cat_name ?? 'Category Name' }}</p>
                    </div>
                </div>
                @php
                    $imageCount++;
...
<div class="flex justify-center">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-4 max-w-5xl">
        @foreach ($Filtercategories->slice(2) as $item)
            @php
                $imagePath = public_path('images/' . $item->cat_image);
                $imageSize = getimagesize($imagePath);
                $imageWidth = $imageSize[0];
            ...
            @if ($imageWidth > 200 && $imageHeight > 200)
                <div>
                    <img src="{{ asset('images/' . $item->cat_image) }}" class="h-64 w-full object-cover"
                        alt="{{ $item->cat_name ?? 'Category Image' }}">
                </div>
            @endif
        @endforeach
...
<div class="flex justify-center px-4">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-screen-xl">
        @foreach ($Filtercategories->slice(4) as $item)
            <div>
                <img src="{{asset('images/'.$item->cat_image)}}"
